...
## Step 1 Taking a Sensing Moisture @showdialog
...
In this tutorial, we will use the soil moisture sensor to trigger an event. The value for these reading will be stored and then displayed on the micro:bit computer. We will also add an extension to MakeCode, to help us access the DHT11 sensor readings.  Extensions only need to be added once.
...
## Step 5 Create a Variable (Setting the Environment)
...
1. Click ``||Variables: Make a Variable...||`` to create a variable and call it SoilMoisture.
2. Go to ``||Variables: Variables||`` and place the ``||Variables:Set SoilMoisture to||`` block inside the ``||Basic:on Start||`` block.
![Making a variable](https://raw.githubusercontent.com/EarthEdSTEM/earthed-iot-programs-tutorials/master/Images/T_Soil_Moisture/IoT_Soil_Moisture_Create_Variable.png)
